...
{/* | `autoStart` | boolean | Inicia a gravação automaticamente, sem a necessidade de clicar no botão de iniciar. |
| `finishEventName` | string | Nome do evento que irá finalizar a gravação. |
| `autoFinishOnEvent` | boolean | Finaliza a gravação automaticamente quando o evento informado no `finishEventName` for emitido. | */}
...
<Info>
A funcionalidade de desabilitar a seleção de tipo de consulta (`skipConsultationType`) está disponível em versão de teste na versão **2.5.0** do componente.
</Info>
...
O componente pode ser configurado para enviar dados para prescrição médica utilizando a propriedade `prescription-data`. Isso permite que você envie dados adicionais para que a prescrição médica seja gerada com base nos dados enviados.

<Info>
A funcionalidade de envio de dados para prescrição médica (`prescription-data`) está disponível para teste a partir da versão **2.5.0** do componente.
</Info>

### Mevo

Para a plataforma MEVO, você deve enviar os seguintes dados:
- `provider`: "MEVO"
- `externalReference`: Identificador único na plataforma MEVO
